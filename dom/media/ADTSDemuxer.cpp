...
#ifdef PR_LOGGING
//PRLogModuleInfo* gMediaDemuxerLog; fix fixfix
//#define ADTSLOG(msg, ...) \
//  PR_LOG(gMediaDemuxerLog, PR_LOG_DEBUG, ("ADTSDemuxer " msg, ##__VA_ARGS__))
//#else
...
ADTSTrackDemuxer::ScanUntil(const media::TimeUnit& aTime)
{
  ...
  while (SkipNextFrame(FindNextFrame()) && Duration(mFrameIndex + 1) < aTime) {
    ADTSLOG("ScanUntil* avgFrameLen=%f mNumParsedFrames=%" PRIu64
...
ADTSTrackDemuxer::GetSamples(int32_t aNumSamples)
{
  ...
  ADTSLOG("GetSamples() End mSamples.Size()=%d aNumSamples=%d mOffset=%" PRIu64
...
ADTSTrackDemuxer::FindNextFrame(bool findFirstFrame /*= false*/)
{
  ...
  ADTSLOG("FindNext() Begin mOffset=%" PRIu64 " mNumParsedFrames=%" PRIu64
  ...
  ADTSLOG("FindNext() End mOffset=%" PRIu64 " mNumParsedFrames=%" PRIu64
...
ADTSTrackDemuxer::SkipNextFrame(const adts::Frame& aFrame)
{
  ...
  ADTSLOG("SkipNext() End mOffset=%" PRIu64 " mNumParsedFrames=%" PRIu64
          " mFrameIndex=%" PRId64 " mTotalFrameLen=%" PRIu64
          " mSamplesPerFrame=%d mSamplesPerSecond=%d mChannels=%d",
          mOffset, mNumParsedFrames, mFrameIndex, mTotalFrameLen,
          mSamplesPerFrame, mSamplesPerSecond, mChannels);
...
ADTSTrackDemuxer::GetNextFrame(const adts::Frame& aFrame)
{
  ...
  ADTSLOG("GetNext() End mOffset=%" PRIu64 " mNumParsedFrames=%" PRIu64
          " mFrameIndex=%" PRId64 " mTotalFrameLen=%" PRIu64
          " mSamplesPerFrame=%d mSamplesPerSecond=%d mChannels=%d",
          mOffset, mNumParsedFrames, mFrameIndex, mTotalFrameLen,
          mSamplesPerFrame, mSamplesPerSecond, mChannels);
...
ADTSTrackDemuxer::FrameIndexFromOffset(int64_t aOffset) const
{
  ...
  ADTSLOG("FrameIndexFromOffset(%" PRId64 ") -> %" PRId64, aOffset, frameIndex);
...
ADTSTrackDemuxer::FrameIndexFromTime(const media::TimeUnit& aTime) const
{
  ...
  ADTSLOG("FrameIndexFromOffset(%fs) -> %" PRId64, aTime.ToSeconds(), frameIndex);
  return std::max<int64_t>(0, frameIndex);
}
...
{
  ADTSLOG("ADTSTrackDemuxer::Read(%p %" PRId64 " %d)", aBuffer, aOffset, aSize);
  ...
  uint32_t read = 0;
  ADTSLOG("ADTSTrackDemuxer::Read        -> ReadAt(%d)", aSize);
