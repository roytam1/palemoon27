                       ...
                       SSLHashType *hash, const ssl3BulkCipherDef **cipher)
{
    ...
    vrange.min = vrange.max = version;
    if (!ssl3_CipherSuiteAllowedForVersionRange(cipherSuite, &vrange)) {
        PORT_SetError(SEC_ERROR_INVALID_ARGS);
        return SECFailure;
    }
    suiteDef = ssl_LookupCipherSuiteDef(cipherSuite);
    cipherDef = ssl_GetBulkCipherDef(suiteDef);
                ...
                SSLAeadContext **ctx)
{
    ...
    rv = tls13_GetHashAndCipher(version, cipherSuite,
                                          &hash, &cipher);
    if (rv != SECSuccess) {
    ...
    labelLen = labelPrefixLen + strlen(ivSuffix);
    ivLen = cipher->iv_size + cipher->explicit_nonce_size;
              ...
              PRUint8 *out, unsigned int *outLen, unsigned int maxOut)
{
    ...
    // Setup the nonce.
    rv = sslBuffer_AppendNumber(&nonceBuf, counter, sizeof(counter));
    if (rv != SECSuccess) {
        PORT_Assert(0);
        return SECFailure;
    }
